...
namespace UltraPing.NET
{
    class TraceRoute
    {
        public String startTrace(String address) {
           byte[] data = new byte[1024];
          int recv, timestart, timestop;
          String result = "";
          Socket host = new Socket(AddressFamily.InterNetwork, SocketType.Raw, ProtocolType.Raw);
          ...
          for (int i = 1; i < 50; i++)
          {
             ...
             try
             {
                ...
                result += "hop " + i + ": response from " + ep.ToString() + " " + timeDiff + " ms";
                }
                
                if (response.Type == 0)
                {
                   result += ep.ToString() + "reached in " + i + " hops " + timeDiff + " ms.";
                   break;
                }
                badcount = 0;
